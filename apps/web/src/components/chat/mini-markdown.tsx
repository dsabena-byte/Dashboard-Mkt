...
// separador (---), links http(s) y links internos a /guia (Proceso Estratégico). Las imágenes se descartan. (Portado de BIP.)
  ...
  for (const m of clean.matchAll(re)) {
    ...
    if (tok.startsWith("**")) out.push(<strong key={`${kb}b${i++}`} className="font-semibold text-foreground">{tok.slice(2, -2)}</strong>);
      ...
      out.push(/^https?:\/\//.test(href)
        ...
        : /^\/guia(\/[a-z0-9-]+)?$/.test(href) // Proceso Estratégico: link interno a un módulo (misma pestaña)
