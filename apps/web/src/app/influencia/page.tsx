...
import { getUgcTestEntries } from "@/lib/ugc-testing-queries";
...
export default async function InfluenciaPage() {
  const [rows, ugcCreatives, ugcAnalysis, ugcEntries, lastUpdated] = await Promise.all([
    ...
    safe(getUgcTestEntries(), [] as Awaited<ReturnType<typeof getUgcTestEntries>>),
    safe(maxUpdatedAt("pauta_performance"), null),
  ]);

  return <InfluenciaClient rows={rows} ugcCreatives={ugcCreatives} ugcAnalysis={ugcAnalysis} ugcEntries={ugcEntries} lastUpdated={lastUpdated} />;
}
