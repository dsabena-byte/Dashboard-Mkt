...
export async function POST(request: Request) {
  try {
    const { guion, genero, escenario, duracion, edad, vestimenta } = (await request.json()) as { guion?: string; genero?: string; escenario?: string; duracion?: number; edad?: string; vestimenta?: string };
    if (!guion?.trim()) return NextResponse.json({ ok: false, error: "Falta el guion." }, { status: 400 });
    const h = await generarVideoUgcSeedanceSubmit(guion, genero, escenario, duracion, edad, vestimenta);
