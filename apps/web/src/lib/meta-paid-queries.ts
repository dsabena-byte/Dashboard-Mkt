...
export interface MetaPaidCreativeRow {
  ...
  // Estado actual: true = effective_status ACTIVE (corriendo); false = pausada/archivada;
  // null = el sync todavía no trajo el estado (piezas legacy / meses sin re-sync).
  activa: boolean | null;
}
...
function mapRow(r: DbRow): MetaPaidCreativeRow {
  return {
    ...
    activa: r.effective_status == null ? null : r.effective_status === "ACTIVE",
  };
}
