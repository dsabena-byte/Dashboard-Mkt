...
// ruido de otras categorías (ej. "samsung" incluye celulares/TVs: validado, daba
// 86 vs Drean 10, mientras que "lavarropas samsung" = 14.800 vs "lavarropas
// drean" = 33.100). El SEO (Módulo B) usa el dominio de cada marca.
...
export interface Marca {
  nombre: string;
  // Slug en minúsculas para armar las keywords ("drean", "whirlpool", ...).
  slug: string;
  // Dominio para el Módulo B (SEO). Las marcas emergentes (Hisense/Midea) o las
  // globales con path por país (Samsung/LG) se confirman contra data real al
  // implementar el sync de SEO; para el Módulo A (demanda) el dominio no se usa.
  dominio: string;
  // Marca emergente: se espera poca data hoy; el interés es trackear su rampa.
  emergente?: boolean;
}
...
export interface KeywordDef {
  keyword: string;
  marca: string | null; // null = genérico de categoría (no atado a marca)
  categoria: Categoria;
  generico: boolean;
}

// Universo de keywords para el Módulo A (demanda / Share of Search):
//  - genéricas por categoría: "lavarropas", "heladera", "cocina"
//  - de marca × categoría: "lavarropas drean", "heladera samsung", ...
// Forma canónica: "{termino} {marca}" (validado: es la de mayor volumen real).
